import React, { useEffect } from 'react'
import { Link } from 'react-router-dom'
import './Home.css'
import { Logo } from '../images/Netflix'
import {
  Button,
  ConnectButton,
  Icon,
  Modal,
  Tab,
  TabList,
  useNotification,
} from 'web3uikit'
import { movies } from '../helpers/library'
import { useState } from 'react'
import { useMoralis } from 'react-moralis'

const Home = () => {
  const [visible, setVisible] = useState(false)
  const [boolSelectedMovieInMyList, setSelectedMovieInMyList] = useState(false)
  //const [isSelectedMovieInMyList] = useState()
  const [selectedFilm, setSelectedFilm] = useState()
  const [myMovies, setMyMovies] = useState()
  const { isAuthenticated, Moralis, account } = useMoralis()

  useEffect(() => {
    fetchMyList()
  }, [account])

  useEffect(() => {
    isSelectedMovieInMyList()
  }, [myMovies])

  useEffect(() => {
    isSelectedMovieInMyList()
  }, [selectedFilm])

  useEffect(() => {
    isSelectedMovieInMyList()
  }, [account])

  const dispatch = useNotification()

  function isSelectedMovieInMyList() {
    setSelectedMovieInMyList(false);
    myMovies && selectedFilm &&
    myMovies.forEach((e) => {
      if (e.Name === selectedFilm.Name) {
        setSelectedMovieInMyList(true);
      }
    })    
  }

  const fetchMyList = async function () {
    await Moralis.start({
      serverUrl: 'https://ieqhn3yzixu8.usemoralis.com:2053/server',
      appId: 'tcVdxyRYIMMGBTsdCWdnsIQTw7jshqkjUsJQGGPM',
    }) //if getting errors add this

    try {
      const theList = await Moralis.Cloud.run('getMyList', { addrs: account })

      const filterdA = movies.filter(function (e) {
        return theList.indexOf(e.Name) > -1
      })

      setMyMovies(filterdA)
    } catch (error) {
      console.error(error)
    }
  }

  const handleNewNotification = () => {
    dispatch({
      type: 'error',
      message: 'Please Connect Your Crypto Wallet',
      title: 'Not Authenticated',
      position: 'topL',
    })
  }

  const handleAddNotification = () => {
    dispatch({
      type: 'success',
      message: 'You add a new Movie on your List',
      title: 'New Movie Added',
      position: 'topL',
    })
  }

  const handleRemoveNotification = () => {
    dispatch({
      type: 'success',
      message: 'You remove the Movie from your List',
      title: 'Movie Removed',
      position: 'topL',
    })
  }

  return (
    <>
      <div className="logo">
        <Logo />
      </div>

      <div className="connect">
        <Icon fill="#FFFFFF" size={24} svg="bell" />
        <ConnectButton />
      </div>
      <div className="topBanner">
        <TabList defaultActiveKey={1} tabStyle="bar">
          <Tab tabKey={1} tabName={'Movies'}>
            <div className="scene">
              <img
                src={movies[0].Scene}
                className="sceneImg"
                alt={movies[0].Name}
              ></img>
              <img
                className="sceneLogo"
                src={movies[0].Logo}
                alt={movies[0].Name}
              ></img>
              <p className="sceneDesc">{movies[0].Description}</p>
              <div className="playButton">
                <Button
                  icon="chevronRightX2"
                  iconLayout="leading"
                  id="test-button-translucent-icon-large"
                  onClick={function noRefCheck() {}}
                  theme="secondary"
                  type="button"
                  text="Play"
                />

                <Button
                  icon="plus"
                  iconLayout="leading"
                  id="test-button-translucent-icon-large"
                  onClick={() => {
                    console.log(myMovies)
                  }}
                  size="large"
                  text="Add to My List"
                  theme="translucent"
                />
              </div>
            </div>

            <div className="title">Movies</div>

            <div className="thumbs">
              {movies &&
                movies.map((e) => {
                  return (
                    <img
                      src={e.Thumnbnail}
                      className="thumbnail"
                      alt={e.Name}
                      onClick={() => {
                        setSelectedFilm(e)
                        setVisible(true)
                      }}
                    />
                  )
                })}
            </div>
          </Tab>
          <Tab tabKey={2} tabName={'Series'} isDisabled={true}></Tab>
          <Tab tabKey={3} tabName={'MyList'}>
            <div className="ownListContent">
              <div className="title">Your Library</div>
              {myMovies && isAuthenticated ? (
                <>
                  <div className="ownThumbs">
                    {myMovies.map((e) => {
                      return (
                        <img
                          src={e.Thumnbnail}
                          className="thumbnail"
                          alt={e.Name}
                          onClick={() => {
                            setSelectedFilm(e)
                            setVisible(true)
                          }}
                        />
                      )
                    })}
                  </div>
                </>
              ) : (
                <div className="ownThumbs">
                  You need to Authentificate to View Your Own List
                </div>
              )}
            </div>
          </Tab>
        </TabList>

        {selectedFilm && (
          <div className="modal">
            <Modal
              isVisible={visible}
              hasFooter={false}
              width="1000px"
              onCloseButtonPressed={() => {
                setVisible(false)
              }}
            >
              <div className="modalContent">
                <img
                  src={selectedFilm.Scene}
                  className="modalImg"
                  alt={selectedFilm.Name}
                ></img>
                <img
                  className="modalLogo"
                  src={selectedFilm.Logo}
                  alt={selectedFilm.Name}
                ></img>
                <div className="modalPlayButton">
                  {isAuthenticated ? (
                    <>
                      <Link to="/player" state={selectedFilm.Movie}>
                        <Button
                          icon="chevronRightX2"
                          iconLayout="leading"
                          id="test-button-translucent-icon-large"
                          onClick={function noRefCheck() {}}
                          theme="secondary"
                          type="button"
                          text="Play"
                        />
                      </Link>
                      {boolSelectedMovieInMyList ? (
                       
                        <>
                          <Button
                            icon="minus"
                            iconLayout="leading"
                            id="test-button-translucent-icon-large"
                            onClick={async () => {
                              await Moralis.Cloud.run('removeFromMyList', {
                                addrs: account,
                                oldFav: selectedFilm.Name,
                              })
                              handleRemoveNotification()
                              fetchMyList()
                            }}
                            size="large"
                            text="Remove from My List"
                            theme="translucent"
                          />
                        </>
                      ) : (
                        <>
                          <Button
                            icon="plus"
                            iconLayout="leading"
                            id="test-button-translucent-icon-large"
                            onClick={async () => {
                              await Moralis.Cloud.run('updateMyList', {
                                addrs: account,
                                newFav: selectedFilm.Name,
                              })
                              handleAddNotification()
                              fetchMyList()
                            }}
                            size="large"
                            text="Add to My List"
                            theme="translucent"
                          />
                        </>
                      )}
                    </>
                  ) : (
                    <>
                      <Button
                        icon="chevronRightX2"
                        iconLayout="leading"
                        id="test-button-translucent-icon-large"
                        onClick={handleNewNotification}
                        theme="secondary"
                        type="button"
                        text="Play"
                      />
                      <Button
                        icon="plus"
                        iconLayout="leading"
                        id="test-button-translucent-icon-large"
                        onClick={handleNewNotification}
                        size="large"
                        text="Add to My List"
                        theme="translucent"
                      />
                    </>
                  )}
                </div>
                <div className="movieInfo">
                  <div className="description">
                    <div className="details">
                      <span>{selectedFilm.Year}</span>
                      <span>{selectedFilm.Duration}</span>
                    </div>
                    {selectedFilm.Description}
                  </div>
                  <div className="detailedInfo">
                    Genre:
                    <span className="deets">{selectedFilm.Genre}</span>
                    <br />
                    Actors:
                    <span className="deets">{selectedFilm.Actors}</span>
                  </div>
                </div>
              </div>
            </Modal>
          </div>
        )}
      </div>
    </>
  )
}

export default Home
